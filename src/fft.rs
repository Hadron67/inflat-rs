use std::{
    cmp::max, mem::MaybeUninit, sync::{Arc, Mutex}
};
...
use crate::{field::{Dim, LatticeLike, LatticeMutLike, Vec3i}, lat::{BoxLattice, Lattice}};
...
impl<const D: usize, T> DftNDPlan<D, T> where T: FftNum + Float + FloatConst + Send + Sync {
    pub fn new(dims: [usize; D], direction: FftDirection) -> Self {
        let mut planner = FftPlanner::new();
        let plans = dims.map(|dim|planner.plan_fft(dim, direction));
        Self { plans, scratch: Mutex::new(vec![]) }
    }
    ...
    pub fn transform_one_axis_inplace(&self, field: &mut BoxLattice<D, Complex<T>>, axis: usize) {
        ...
        field.par_axis_co_iter_mut(axis).for_each(move |(_, mut arr)|{
...
mod tests {
    ...
    use crate::{fft::DftNDPlan, field::{Dim, Lattice, LatticeLike, LatticeMutLike}, lat::{BoxLattice, LatticeMut, LatticeSupplier}, util::VecN};
    ...
    fn fft2() -> anyhow::Result<()> {
        ...
        let mut rand = random::default(1);
        input.for_each(|ptr, index, coord|*ptr = (coord[1] as f64).into());
        ...
        plan.transform_inplace(&mut transformed);
        let diff = transformed.view().mul_scalar((1.0 / vol).into()).flip().plus(input.view().mul_scalar((-1.0).into())).map(|f|f.abs() * f.abs()).average();
        println!("diff = {}", diff);
