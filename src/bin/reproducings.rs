...
fn run_simulation<T1: Fn(f64) -> f64 + Send + Sync, T2: Fn(f64) -> f64 + Send + Sync>(name: &str, params: FullSimulateParams<T1, T2>, mass_scale: f64, force_rerun: bool) {
    ...
    if force_rerun || !npy_path.exists() {
        ...
        let out_arr = Array2::from_shape_vec((out_arr.len() / 7, 7), out_arr).unwrap().reversed_axes();
    ...
    plot.add_trace(Scatter::new(efoldings.to_vec(), phi_data.to_vec()).x_axis("x2").y_axis("y2").name("\\phi"));
    plot.add_trace(Scatter::new(efoldings.to_vec(), phi_d_data.to_vec()).x_axis("x3").y_axis("y3").name("\\dot\\phi / m"));
    plot.add_trace(Scatter::new(efoldings.to_vec(), slowroll_epsilon_data.to_vec()).x_axis("x4").y_axis("y4").name("\\epsilon"));
    plot.add_trace(Scatter::new(efoldings.to_vec(), hamitonian_data.to_vec()).x_axis("x5").y_axis("y5").name("Hamitonian"));
    plot.add_trace(Scatter::new(efoldings.to_vec(), hubble_constraint_data.to_vec()).x_axis("x6").y_axis("y6").name("Hubble constraint"));
    plot.set_layout(Layout::new().grid(LayoutGrid::new().rows(2).columns(3).pattern(GridPattern::Independent)));
...
fn run_oscillator() {
    ...
    plot.add_trace(Scatter::new(x_data.clone(), hamitonian_data).name("H").x_axis("x2").y_axis("y2"));
    plot.add_trace(Scatter::new(x_data.clone(), scale_factor_data).name("a").x_axis("x3").y_axis("y3"));
    plot.add_trace(Scatter::new(x_data, hubble_constraint_data).name("hc").x_axis("x4").y_axis("y4"));
    plot.set_layout(Layout::new().grid(LayoutGrid::new().rows(2).columns(2).pattern(GridPattern::Independent)).height(900));
